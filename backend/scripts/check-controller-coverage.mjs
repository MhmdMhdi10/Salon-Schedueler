...
const routeCoverage =
  requiredRoutes.size === 0 ? 0 : (coveredRoutes.size / requiredRoutes.size) * 100;

console.log(`Backend Cucumber scenarios: ${passed.length}/${scenarios.length} passed`);
console.log(`Backend controller routes: ${coveredRoutes.size}/${requiredRoutes.size} exercised`);
console.log(`Controller endpoint coverage: ${routeCoverage.toFixed(2)}%`);

if (missing.length) console.error(`Missing controller routes: ${missing.join(' | ')}`);
if (unknown.length) console.error(`Unknown controller routes: ${unknown.join(' | ')}`);
if (failed.length)
