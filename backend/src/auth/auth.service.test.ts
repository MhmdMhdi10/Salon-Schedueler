...
function createMockPrisma() {
  ...
  return {
    ...
    _platformAdminStore: platformAdminStore,
    otp: {
    ...
    platformAdmin: {
      findFirst: jest.fn(async ({ where }: any) => {
        return (
          platformAdminStore.find(
            (admin) =>
              admin.phone === where.phone &&
              (where.active === undefined || admin.active === where.active),
          ) || null
        );
      }),
      findUnique: jest.fn(async ({ where }: any) => {
        return platformAdminStore.find((admin) => admin.id === where.id) || null;
      }),
...
describe('AuthService', () => {
  ...
  describe('verifyOtp', () => {
    ...
    it('should reject expired OTP (R1.3)', async () => {
      ...
      // Extract code
      const smsMessage = smsProvider.calls[0].message;
      const code = extractCode(smsMessage);
  ...
  describe('refresh', () => {
    ...
    it('should reject an access token presented as a refresh token', async () => {
      const accessToken = jwt.sign(
        { sub: 'existing-customer', type: 'access' },
        config.jwtRefreshSecret,
      );
      await expect(authService.refresh(accessToken)).rejects.toMatchObject({
        code: 'INVALID_TOKEN',
      });
    });

    it('should re-resolve staff permissions when refreshing', async () => {
      const phone = '09123456789';
      prisma._customerStore.push({ id: 'customer-1', phone });
      ...
      await authService.requestOtp(phone);
      const code = extractCode(smsProvider.calls[0].message);
      const tokens = await authService.verifyOtp(phone, code);

      // A role removed after login must not survive token refresh.
      prisma._staffStore[0].active = false;
      const refreshed = await authService.refresh(tokens.refreshToken);
      const decoded = jwt.verify(refreshed.accessToken, config.jwtAccessSecret) as jwt.JwtPayload;
      expect(decoded.sub).toBe('customer-1');
      expect(decoded.role).toBeUndefined();
      expect(decoded.staffMemberId).toBeUndefined();
    });
  ...
  describe('invalidate-previous-on-reissue (R1.5)', () => {
    it('should invalidate previous OTP when a new one is requested', async () => {
      ...
      // Request first OTP
      await authService.requestOtp(phone);
      const firstCode = extractCode(smsProvider.calls[0].message);
