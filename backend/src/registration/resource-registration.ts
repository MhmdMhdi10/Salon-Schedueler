...
export class ResourceRegistration {
  ...
  async registerChair(
    ...
    const chair = await this.prisma.chair.create({
      data: {
        ...
        ...(options.kind === 'mobile' && options.mobileStaffId
          ? { mobileStaff: { connect: { id: options.mobileStaffId } } }
          : options.assignedStaffId
          ? { assignedStaff: { connect: { id: options.assignedStaffId } } }
          : {}),
      },
    });
  ...
  /**
   * Ensure a salon has the capacity lanes required by its operating model.
   * Existing chairs and historical appointments are preserved; this only adds
   * missing lanes when an owner switches between fixed, rented, and mobile work.
   */
  async ensureWorkModeCapacity(salonId: string, workMode: string): Promise<void> {
    const staff = await this.prisma.staffMember.findMany({
      where: { salonId, active: true, role: { in: ['Owner', 'Stylist'] } },
    });
    let chairs = await this.prisma.chair.findMany({
    ...
    const needsMobile = workMode === 'mobile' || workMode === 'hybrid';
    const needsPhysical = !needsMobile || workMode === 'hybrid';
    ...
    if (needsPhysical && !chairs.some((chair) => chair.kind === 'physical')) {
      const chair = await this.prisma.chair.create({
        data: { salonId, name: 'صندلی اصلی', kind: 'physical' },
      });
      chairs = [...chairs, { ...chair, assignedStaff: null, mobileStaff: null }];
      await this.prisma.workingHours.createMany({
        data: defaultHours.map((hour) => ({ ...hour, ownerId: chair.id })),
      });
    }
    ...
    if (
      ...
      staff.length === 1 &&
      !staff[0].assignedChairId
    ) {
      const physicalChairs = chairs.filter((chair) => chair.kind === 'physical');
      if (physicalChairs.length === 1) {
        await this.prisma.staffMember.update({
          where: { id: staff[0].id },
          data: { assignedChairId: physicalChairs[0].id },
        });
      }
    }
    ...
    for (const member of staff) {
      const hasLane = chairs.some(
        (chair) => chair.kind === 'mobile' && chair.mobileStaff?.id === member.id,
      );
      if (hasLane) continue;
      const chair = await this.prisma.chair.create({
        data: {
      ...
      const staffHours = await this.prisma.workingHours.findMany({
        where: { ownerKind: 'staff', ownerId: member.id },
      });
      await this.prisma.workingHours.createMany({
        data: (staffHours.length > 0
          ...
          : defaultHours.map((hour) => ({ ...hour, ownerId: chair.id }))) as never,
      });
    }
  }
