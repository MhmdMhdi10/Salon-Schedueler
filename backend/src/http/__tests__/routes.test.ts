...
function joinedSetCookieHeader(value: string | string[] | undefined): string {
...
describe('HTTP routes', () => {
  ...
  describe('POST /api/auth/otp/verify', () => {
    ...
    it('returns only the access token and sets an HttpOnly refresh cookie for web clients', async () => {
      fake.authService.verifyOtp.mockResolvedValue({
      ...
      const cookie = joinedSetCookieHeader(res.headers['set-cookie']);
      expect(cookie).toContain('salon_refresh=refresh-web');
      expect(cookie).toContain('HttpOnly');
      expect(cookie).toContain('SameSite=Lax');
    });

    it('does not trust the mobile header on a browser-origin request', async () => {
      fake.authService.verifyOtp.mockResolvedValue({
      ...
      expect(joinedSetCookieHeader(res.headers['set-cookie'])).toContain(
        'salon_refresh=refresh-browser',
      );
    });
  });

  describe('POST /api/auth/refresh', () => {
    it('reads the refresh cookie for web clients and rotates it', async () => {
      fake.authService.refresh.mockResolvedValue({
      ...
      const res = await request(app)
        ...
        .set('X-Auth-Client', 'web')
        .set('Cookie', 'salon_refresh=refresh-web')
        .send({});

      expect(res.status).toBe(200);
      expect(res.body).toEqual({ accessToken: 'access-rotated' });
      expect(fake.authService.refresh).toHaveBeenCalledWith('refresh-web');
      expect(joinedSetCookieHeader(res.headers['set-cookie'])).toContain(
        'salon_refresh=refresh-rotated',
      );
    });

    it('keeps token-body refresh responses for explicit mobile clients', async () => {
      fake.authService.refresh.mockResolvedValue({
      ...
      const res = await request(app)
        .post('/api/auth/refresh')
        .set('X-Auth-Client', 'mobile')
        .send({ refreshToken: 'refresh-mobile-old' });

      expect(res.status).toBe(200);
      expect(res.body).toEqual({ accessToken: 'access-mobile', refreshToken: 'refresh-mobile' });
      expect(fake.authService.refresh).toHaveBeenCalledWith('refresh-mobile-old');
    });

    it('does not return a refresh token to a browser that spoofs the mobile header', async () => {
      fake.authService.refresh.mockResolvedValue({
        accessToken: 'access-browser',
        refreshToken: 'refresh-browser',
      });
      ...
      const res = await request(app)
        ...
        .set('X-Auth-Client', 'mobile')
        .set('Origin', 'http://localhost:5273')
        .set('Cookie', 'salon_refresh=refresh-cookie')
        .send({ refreshToken: 'refresh-spoofed' });

      expect(res.status).toBe(200);
      expect(res.body).toEqual({ accessToken: 'access-browser' });
      expect(fake.authService.refresh).toHaveBeenCalledWith('refresh-cookie');
      expect(joinedSetCookieHeader(res.headers['set-cookie'])).toContain(
        'salon_refresh=refresh-browser',
      );
    });
  ...
  describe('POST /api/auth/logout', () => {
    it('clears the browser refresh cookie', async () => {
      const res = await request(app).post('/api/auth/logout').set('X-Auth-Client', 'web');

      expect(res.status).toBe(204);
      const cookie = joinedSetCookieHeader(res.headers['set-cookie']);
      expect(cookie).toContain('salon_refresh=;');
  ...
  describe('POST /api/appointments/:id/approve', () => {
    const pendingAppointment = {
      id: 'appt-pending',
      salonId: 'salon-1',
      staffMemberId: 'staff-Stylist-1',
      status: 'pending',
    };

    it('blocks a Stylist until the owner grants approval permission', async () => {
      fake.calendarService.getAppointmentById.mockResolvedValue(pendingAppointment);
      fake.resourceRegistration.getStaffMember.mockResolvedValue({
        id: 'staff-Stylist-1',
        salonId: 'salon-1',
      ...
      const res = await request(app)
        .post(`/api/appointments/${pendingAppointment.id}/approve`)
        .set('Authorization', `Bearer ${staffToken('Stylist')}`);

      expect(res.status).toBe(403);
      expect(res.body).toEqual({ code: 'FORBIDDEN' });
      expect(fake.bookingFlow.approve).not.toHaveBeenCalled();
    });

    it('allows a Stylist to approve only their own booking after permission is granted', async () => {
      fake.calendarService.getAppointmentById.mockResolvedValue(pendingAppointment);
      fake.resourceRegistration.getStaffMember.mockResolvedValue({
        id: 'staff-Stylist-1',
        salonId: 'salon-1',
      ...
      fake.bookingFlow.approve.mockResolvedValue({ id: pendingAppointment.id, status: 'confirmed' });
      ...
      expect(res.body).toEqual({
        status: 'confirmed',
        appointment: { id: pendingAppointment.id, status: 'confirmed' },
      });
      expect(fake.bookingFlow.approve).toHaveBeenCalledWith(pendingAppointment.id);
    });

    it('keeps Owner override regardless of the stylist flag', async () => {
      fake.calendarService.getAppointmentById.mockResolvedValue({
        ...pendingAppointment,
        staffMemberId: 'staff-Stylist-2',
      });
      fake.bookingFlow.approve.mockResolvedValue({ id: pendingAppointment.id, status: 'confirmed' });
