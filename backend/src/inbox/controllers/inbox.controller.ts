...
import type { HttpPrincipal } from '../../http/middleware/auth.js';
...
export class InboxController {
  ...
  public router(): Router {
    if (!this.requireRole) {
      throw new Error('InboxController.router requires a role guard');
    }
    return inboxRouter(this.services, this.requireRole);
  }

  public wsHandle(jwtAccessSecret: string) {
    return makeWsInboxHandle(this.services, jwtAccessSecret);
  }
}
