...
export class AvailabilityConfig {
  ...
  async getSalonWorkMode(salonId: string): Promise<string> {
    const salon = (await this.prisma.salon.findUnique({
      where: { id: salonId },
      select: { workMode: true } as never,
    })) as unknown as { workMode: string } | null;
    if (!salon) throw new Error('Salon not found');
    return salon.workMode;
  }

  /** Change work mode; capacity provisioning is handled by the route/service layer. */
  async setSalonWorkMode(salonId: string, workMode: string): Promise<void> {
    await this.prisma.salon.update({
      where: { id: salonId },
      data: { workMode } as never,
    });
  }
  ...
  /** Grant or revoke a stylist's ability to approve/reject their own pending appointments. */
  async setStaffCanApproveOwnAppointments(
    staffMemberId: string,
    allowed: boolean,
  ): Promise<void> {
    await this.prisma.staffMember.update({
      where: { id: staffMemberId },
      data: { canApproveOwnAppointments: allowed },
    });
  }
