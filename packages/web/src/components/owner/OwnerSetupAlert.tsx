...
export function OwnerSetupAlert({ salonId, refreshKey }: { salonId: string; refreshKey?: string }) {
  ...
  const load = useCallback(async () => {
    try {
      ...
      const activeStaff = staffResult.staff.filter(
  ...
  if (loadError && issues.length === 0) {
    return (
      <aside className="sticky top-0 z-sticky mb-4 flex items-center justify-between gap-3 rounded-xl border border-warning/35 bg-warning/10 px-4 py-3 shadow-2" role="status">
        <span className="text-sm font-bold text-warning">بررسی آماده‌بودن رزرو انجام نشد.</span>
        <Button type="button" variant="ghost" size="md" onClick={() => void load()} startIcon={<RefreshCw className="h-4 w-4" />}>
          بررسی دوباره
        </Button>
      </aside>
    );
  ...
  return (
    <aside
      ...
      className="sticky top-0 z-sticky mb-4 overflow-hidden rounded-2xl border border-danger/40 bg-danger/10 shadow-2 backdrop-blur"
      aria-labelledby="owner-setup-alert-title"
    >
      <div className="flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex min-w-0 items-start gap-3">
          <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-danger text-white shadow-1">
            <TriangleAlert className="h-5 w-5" aria-hidden="true" />
          </span>
          <div>
            <strong id="owner-setup-alert-title" className="block text-sm font-black text-danger">
              رزرو آنلاین نیاز به رسیدگی دارد
            </strong>
            <p className="mt-1 text-xs leading-5 text-text">
              {issues.length === 1 ? issues[0].description : `${issues.length} مورد ضروری مانع رزرو درست مشتری است.`}
            </p>
          </div>
        </div>
      ...
      <ul className="grid border-t border-danger/20 bg-surface/65 sm:grid-cols-2">
        {issues.map((issue) => (
          <li key={issue.key} className="border-b border-danger/15 last:border-b-0 sm:border-e sm:last:border-e-0">
            <Link
              to={issue.to}
              className="flex items-center justify-between gap-3 px-4 py-3 text-sm font-bold text-text no-underline transition hover:bg-danger/5 hover:text-danger"
            >
      ...
      {actionError && <p role="alert" className="border-t border-danger/20 px-4 py-2 text-xs font-bold text-danger">{actionError}</p>}
    </aside>
  );
}
