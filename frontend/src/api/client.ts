...
/**
 * Bootstrap the in-memory access token from the HttpOnly refresh cookie on app
 * load so a page refresh keeps the owner signed in without exposing the long-
 * lived credential to JavaScript (task 5.1; R2.2).
 *
 * Returns `true` when a session was restored. On any failure (missing,
 * expired, or invalid HttpOnly refresh cookie) it clears the stale token and
 * returns `false` so the caller can route to the OTP login. Tokens are never
 * logged.
 */
...
/**
 * Clear the current session: drop the in-memory access token and ask the server
 * to expire the HttpOnly refresh cookie (sign-out, task 5.1).
 */
...
async function request<T>(
  ...
  if (response.status === 401 && canRefresh && path !== '/auth/refresh' && accessToken) {
...
export const adminApi = {
  ...
  getPending: (salonId: string) =>
    request<{ appointments: unknown[]; canApproveOwnAppointments?: boolean }>(
      `/salons/${salonId}/pending`,
    ),
...
export interface ApprovalPolicyStaff {
  ...
  /** Whether this stylist may approve/reject their own pending bookings. */
...
export const approvalPolicyApi = {
  ...
  setStaffCanApproveOwn: (staffId: string, allowed: boolean) =>
    request<{ ok: boolean; allowed: boolean }>(`/staff/${staffId}/approve-own`, {
      method: 'POST',
      body: { allowed },
    }),
};
...
export interface SalonStaff {
  ...
  /** Whether this stylist may approve/reject their own pending bookings. */
  canApproveOwnAppointments?: boolean;
  /** Assigned physical chair for rented-chair workflows, or null for shared. */
