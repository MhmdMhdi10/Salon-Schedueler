 ...
 *  - the scroll landing illustration stylesheet, whose physical coordinates
 *    and pixel geometry describe decorative artwork rather than RTL flow.
 *  - the calendar hero illustration stylesheet, which follows the same
 *    self-contained artwork rules.
 */
function isExcluded(rel: string): boolean {
  ...
  if (rel === 'pages/AraScrollLanding.css') return true;
  if (rel === 'pages/AraCalendarHero.css') return true;
  return false;
}
